    ...
    try:
        with Image.open(path) as raw:
            ...
            for size in SIZES:
                ...
                rows.append({'size': size, 'width': copy.width, 'height': copy.height, 'name': name})
...
def install(app):
    def responsive_image_attrs(url, size=640, sizes='(max-width: 600px) 100vw, 300px'):
        ...
        if parts.path == '/static/img/brand/logo.png':
            attrs.update(width=510, height=382)
        prefix = ('/static/img/brand/' if parts.path == '/static/img/brand/logo.png'
                  else '/static/img/products/')
        if not parts.scheme and not parts.netloc and parts.path.startswith(prefix):
            ...
            if name and '/' not in name and '\\' not in name and name not in ('.', '..'):
                ...
                try:
                    ...
                    if rows and all((folder / row['name']).is_file() for row in rows):
                        ...
                        if chosen.get('height'):
                            attrs['width'] = chosen['width']
                            attrs['height'] = chosen['height']
